...
class Profiler:
    ...
    def _write_json(self, programs: code_manipulation.Function):
        ...
        if score is not None and score >= self._cur_best_program_score:
            ...
            # 保存为 best_result.json
            best_path = os.path.join(self._json_dir, 'best_result.json')
            with open(best_path, 'w') as json_file:
                json.dump(content, json_file)
            # ✅ 打印提示信息
            print(f"[INFO] 保存了当前最优函数（得分：{score}，采样序号：{sample_order}）到 best_result.json")
    ...
    # def save_best_function(self):
    #     if self._cur_best_program_sample_order is not None:
    #         best_fn = self._all_sampled_functions[self._cur_best_program_sample_order]
    #         best_fn_path = os.path.join(self._json_dir, 'best_result.json')
    #         with open(best_fn_path, 'w') as f:
    #             json.dump({
    #                 'sample_order': self._cur_best_program_sample_order,
    #                 'function': str(best_fn),
    #                 'score': best_fn.score
    #             }, f)
